...
Page({
  ...
  onLoad(options) {
    ...
    if (storeId) {
      wx.redirectTo({
        url: `/packageBiz/pages/inspector-check/inspector-check?storeId=${encodeURIComponent(storeId)}`,
      });
      return;
    }
  },
  ...
  onStoreCheckin(e) {
    ...
    if (!storeId) return;
    wx.navigateTo({
      url: `/packageBiz/pages/inspector-store-checkin/inspector-store-checkin?storeId=${encodeURIComponent(storeId)}`,
    });
  },

  goInspectorCheck() {
    wx.navigateTo({ url: '/packageBiz/pages/inspector-check/inspector-check' });
  },

  goAddStore() {
    wx.navigateTo({ url: '/packageBiz/pages/inspector-add-store/inspector-add-store' });
  },

  clearInspectorTestData() {
    wx.showModal({
      title: '清空确认',
      content: '将清空本机全部本地数据（含登录状态、巡店与业务缓存），是否继续？',
      success: (res) => {
        if (!res.confirm) return;
        try {
          clearAppLocalStorage.clearAll();
